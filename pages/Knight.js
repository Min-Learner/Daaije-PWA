    ...
    return (
        <div className="container" style={{backgroundColor: bgc}}>
            <div className='w-[350px] flex justify-center relative'>
                <div className='w-[35px] h-[35px] absolute bg-black text-white flex justify-center items-center left-0 border-none p-1.5 rounded-full transition-all z-50'
                    style={{transform: `translate(${x}, ${y})`}}>
                    <Pirate />
                </div>
                {array.map((e, i) => {
                    return <span key={i} className='odd:translate-y-[40%] border-2 w-[35px] h-[35px] mx-0.5 rounded-full flex justify-center items-center'>{e}</span>
                })}
                <span className='translate-y-[40%] w-[35px] h-[35px] mx-0.5 rounded-full flex justify-center items-center' onClick={() => router.push('/Progress')}>
